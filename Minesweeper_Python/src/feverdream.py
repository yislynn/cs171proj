...
class MyAI(AI):
    ...
    def getAction(self, number: int) -> "Action Object":
        ...
        tile.label = number # set the current tile label
        tile.covered = False # mark as uncovered

        # update effective labels
        self.update_elabels(self.__currX, self.__currY) # update the tile's elabel
        ...
        if (number == len(flagged)):
            for x, y in uncertain: 
                # all tiles in the uncertain list are marked as safe tiles
        ...
        # 2. if tile = number N and has N uncertain (-1) neighbors, all those neighbors are mines (-2)
            ...
            for x, y in uncertain:
                # all tiles in the uncertain list are marked as mines
    ...
    def get_safe(self):
        ...
        for i in range(self.__rows):
            for j in range(self.__cols):
                # get the tiles that are marked as safe and have not been uncovered
    ...
    def get_uncertain(self):
        ...
        for i in range(self.__rows):
            for j in range(self.__cols):
                # get the tiles that are unknown and definitely not mines
                if self.__board[i][j].covered and not self.__board[i][j].flag:
                    idk.append((i, j))
        return idk

    def get_uncovered_neighbors(self, x, y): 
        return list(filter(lambda tile: not self.__board[tile[0]][tile[1]].covered and not self.__board[tile[0]][tile[1]].flag, self.get_neighbors(x,y)))

    def get_covered_neighbors(self, x, y): 
        return list(filter(lambda tile: self.__board[tile[0]][tile[1]].covered, self.get_neighbors(x,y)))

    def get_flagged_neighbors(self, x, y): 
        return list(filter(lambda tile: self.__board[tile[0]][tile[1]].flag, self.get_neighbors(x,y)))
    ...
    def update_elabels(self, x, y):
        ...
        # set to number of remaining mines in the area

        # update tile's neighbors' elabels
        
        # would this be unnecessary? why change the elabels of the surrounding tiles if there was no change made
        # for n in self.get_uncovered_neighbors(x, y):
        #     tile = self.__board[n[0]][n[1]]
        #     tile.elabel = tile.label - len(self.get_flagged_neighbors(n[0], n[1]))
        return
